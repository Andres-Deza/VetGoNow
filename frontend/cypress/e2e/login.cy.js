describe('Login y Registro', () => {
  it('debería permitir login con credenciales válidas', () => {
    cy.visit('/login');
    cy.get('#email').type('juan.1@VetGoNow.com');
  ...
  it('login exitoso con veterinario del seed', () => {
    cy.visit('/login');
    cy.get('#email').type('vet1@VetGoNow.com');
  ...
  it('registro de nuevo usuario', () => {
    ...
    cy.get('#email').type('test.user@VetGoNow.com');
    ...
    it('registro de nuevo veterinario', () => {
      ...
      cy.get('#email').type('test.vet@VetGoNow.com');
